...
Pno *criarLista(int valor) {
    ...
    Pno *ultimo = primeiro; // Criação do ponteiro b que aponta para o primeiro nó
    int i;

    for (i = 2; i <= valor; i++) {
        ultimo->proximo = criarNo(i); // Conecta o próximo nó ao nó atual
        ultimo = ultimo->proximo; // Move o ponteiro b para o próximo nó
    }
...
int Eliminar(Pno **inicio, int valor) {
    ...
    Pno *b = NULL; // Inicializa o nó anterior como null
    ...
    while (atual->proximo != atual) { // Enquanto houver mais de um amigo na lista, continue a eliminação
        if (i == valor) { // Se o contador atingir o valor de eliminação K
            if (b) { // Se houver um nó anterior
                b->proximo = atual->proximo; // O nó anterior aponta para o próximo do nó atual
                free(atual); // Libera a memória
                atual = b->proximo; // Atualiza o nó atual para o próximo
            ...
            i++; // Incrementar +1
            b = atual; // Define o nó anterior como o nó atual
